    ...
    def __init__(self,
        ...
        # CNN layers
            ...
            with tf.name_scope("blstm"):
                ...
                rnn_layers_forward=[tf.nn.rnn_cell.LSTMCell(self.size) for si in [self.size,]*self.num_layers]
                rnn_layers_backward=[tf.nn.rnn_cell.LSTMCell(self.size) for si in [self.size,]*self.num_layers]
                forward_hts, _ = tf.nn.dynamic_rnn(tf.nn.rnn_cell.MultiRNNCell(rnn_layers_forward), self.input_embed, dtype=tf.float32, sequence_length=self.input_lens, scope="LSTM_forward")
                backward_hts_, _ = tf.nn.dynamic_rnn(tf.nn.rnn_cell.MultiRNNCell(rnn_layers_backward), input_embed_reversed, dtype=tf.float32, sequence_length=self.input_lens, scope="LSTM_backward")
                backward_hts = tf.reverse_sequence(backward_hts_, self.input_lens, seq_dim=1)

            self.hts = tf.concat([self.h_conv4,forward_hts, backward_hts], 2)
            self.hts_flat = tf.reshape(self.hts, [-1, size*3])

        # Output Layer
            self.target = tf.placeholder(tf.int32, shape = [self.batch_size, self.max_len], name = "target")
            self.output_embedding = tf.get_variable("output_embedding",[target_vocab_size, 3*self.size], dtype = dtype)
