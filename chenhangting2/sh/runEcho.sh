...
echo "$PYTHONPATH -u $PY --mode TRAIN --model_dir $MODEL_DIR \
...
echo "$PYTHONPATH -u $PY --mode DECODE --model_dir $MODEL_DIR \
...
echo "$PYTHONPATH $SEG $TEST_PATH_FROM $DECODE_OUTPUT $TEST_LOC $SEG_PATH >> $4 2>&1"

echo "$SCORE $SCORE_OPT $SEG_PATH > $SCORE_TXT 2>&1"
echo "tail $SCORE_TXT >> $4"
