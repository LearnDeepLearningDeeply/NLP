...
class CnnModel(object):
    ...
    def __init__(self,
        ...
        # Output Layer
            ...
            self.paramsWithL2norm=tf.trainable_variables(scope='cnn')
            self.paramsWithL2norm=[tf.nn.l2_loss(p) for p in self.paramsWithL2norm]
            self.paramsWithL2norm=tf.accumulate_n(self.paramsWithL2norm)+tf.nn.l2_loss(self.output_embedding)
            self.loss = tf.reduce_mean(-log_likelihood)+self.l2_rate*self.paramsWithL2norm
    ...
    def step(self,session, inputs, targets, lengths, forward_only = False):
        ...
        if forward_only:
            ...
            output_feed += [self.paramsWithL2norm]

        outputs = session.run(output_feed, input_feed, options = self.run_options, run_metadata = self.run_metadata)
        if(self.l2_rate>0 and not forward_only):print("loss {} params weight {}".format(outputs[0],outputs[-1]))
